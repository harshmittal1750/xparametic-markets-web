...
import ThemeProvider, { STYLE_RULES, isThemeDark, useTheme } from './useTheme';
...
describe('ThemeProvider', () => {
  ...
  it('injects transition style rules on color scheme change', () => {
    mockMatchMedia({ matches: true });
    const { result } = renderTheme();

    act(() => result.current.device.setMode('light'));
    expect(document.head.children[0].textContent).toMatch(STYLE_RULES);
    jest.runAllTimers();
    expect(document.head.children[0]).toMatch('');
  });
});
